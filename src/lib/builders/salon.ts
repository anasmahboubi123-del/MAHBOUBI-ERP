...
export function buildSalonCartItem(draft: any): ProductResult {
  ...
  // FIX: تجنب خطأ never باستخدام type assertion
  const cushionsData: any = cushions;
  const cushionsCount = cushionsData?.totalCm
    ? Math.max(1, Math.round(cushionsData.totalCm / 100))
    : (cushionsData?.count || 0);
  const cushionsUnitPrice = cushionsData?.unitPrice || 0;
  ...
  return {
    ...
    details: {
      ...
      cushions: cushionsCount > 0 ? { count: cushionsCount, type: cushionsData?.type || 'قياسي', unitPrice: cushionsUnitPrice, totalPrice: cushionsCost } : undefined,
