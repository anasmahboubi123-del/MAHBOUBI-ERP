...
export default function FabricSelector({
  ...
  async function savePrice() {
    ...
    const { error } = await db()
      .from('fabrics')
      .update({ price_per_meter: price } as any)
      .eq('id', priceEdit.id);

    if (error) {
      toast.error('فشل تحديث الثمن');
      return;
    }

    setFabrics(
      fabrics.map((f) =>
        f.id === priceEdit.id ? { ...f, price_per_meter: price } : f
      )
    );
    ...
    <div>
      ...
      {!loading && fabrics.length === 0 && (
        <p className="text-gray-400">
          لا توجد أثواب - أضفها من واجهة المدير ← الكتالوج
        </p>
      )}
        ...
        {filtered.map((f) => (
          <Card
            ...
            className={`relative text-center ${
              selected?.id === f.id ? 'ring-4 ring-brand-600' : ''
            }`}
          >
            {f.image_url ? (
              ...
              <div className="flex h-32 items-center justify-center rounded-xl bg-gray-100 text-4xl">
                🧵
              </div>
            )}
            <div className="mt-2 font-bold">{f.name}</div>
            {f.color && (
              <div className="text-sm text-gray-500">{f.color}</div>
            )}
            <div className="font-bold text-brand-700">
              {f.price_per_meter} DH/m
            </div>
            <div className="absolute left-2 top-2 flex gap-1">
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  setPriceEdit(f);
                  setNewPrice(String(f.price_per_meter));
                }}
                className="h-8 w-8 rounded-full bg-white/90 shadow"
      ...
      >
        ...
        {priceEdit && adminOk && (
          <div className="space-y-4">
            <Input
              label={`ثمن ${priceEdit.name} (DH/m)`}
              type="number"
              dir="ltr"
              value={newPrice}
              onChange={(e) => setNewPrice(e.target.value)}
            />
            <Button onClick={savePrice} className="w-full">
              💾 حفظ
            </Button>
          </div>
        )}
