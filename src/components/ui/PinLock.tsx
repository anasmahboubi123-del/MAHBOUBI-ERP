...
export default function PinLock({
  ...
  async function submit(value: string) {
    ...
    // 🔑 الحل: تحويل القيمة المسترجعة إلى نص دائماً
    const rawExpected = await getSetting(`pin_${role}`, FALLBACK[role]);
    const expected = String(rawExpected).replace(/^"|"$/g, '').trim(); // إزالة أي علامات تنصيص زائدة
  ...
  return (
    <div className="flex flex-col items-center gap-4 p-4 min-h-screen justify-center bg-gray-50" dir="rtl">
      ...
      <div className="grid grid-cols-3 gap-3" dir="ltr">
        {keys.map((k) => (
          <button
            key={k}
            onClick={() => press(k)}
            className="h-16 w-16 rounded-full bg-white text-2xl font-bold shadow hover:bg-rose-100 active:scale-95 transition-all text-gray-700"
          >
            {k}
          </button>
        ))}
      ...
      {onCancel && (
        <button onClick={onCancel} className="text-gray-500 underline mt-2">إلغاء</button>
      )}
    </div>
  );
}