...
import { ArrowRight, ArrowLeft, RotateCcw, Lock, ShoppingCart } from "lucide-react";
...
export default function Step07_Summary({ draft, onChange, onNext, onBack, onSendToManager }: Step07Props) {
  ...
  const calcCushionDetails = (c: CushionItem) => {
    ...
    const pricePerPillow = fabricCostPerPillow + c.stitchFinalPrice;
    const pillowsTotal = c.count * pricePerPillow;
    const lwataCost = c.hasLwata ? c.count * c.lwataPrice : 0;
    return { fabricCostPerPillow, pricePerPillow, pillowsTotal, lwataCost, total: pillowsTotal + lwataCost };
  };

  const calcCushionItemTotal = (c: CushionItem): number => {
    return calcCushionDetails(c).total;
  };
  ...
  // المرحلة 6: الإضافات ✅ FIXED: يتضمن customItems
  const extrasStage = draft.extrasStage;
  let extrasTotal = 0;
  if (extrasStage) {
    if (extrasStage.stageTotalOverride !== null && extrasStage.stageTotalOverride !== undefined) {
      extrasTotal = extrasStage.stageTotalOverride;
    } else {
      ...
      const tabouriaTotal = extrasStage.tabouria?.enabled
        ...
        : 0;
      const customTotal = (extrasStage.customItems || []).reduce((s, i) => s + i.price, 0);
      extrasTotal = lhayefTotal + tabouriaTotal + customTotal;
    }
  }

  // ✅ FIXED: المجموع الكلي يتضمن ثمن الثوب
      ...
      <div className="sticky top-0 z-30 border-b border-[#1B5E3B]/10 bg-white/80 backdrop-blur-md">
        <div className="mx-auto flex max-w-4xl items-center justify-between px-4 py-4">
          ...
          <div className="text-center">
            <h1 className="text-xl font-bold text-[#0D1F17] md:text-2xl">📋 ملخص الطلبية</h1>
            <p className="mt-0.5 text-sm text-gray-500">المراجعة النهائية قبل الإرسال</p>
          </div>
          <div className="w-20" />
        </div>
      </div>
      ...
      <div className="mx-auto max-w-4xl px-4 py-6 space-y-6">
          ...
          {seddars.length === 0 ? (
            ...
            <>
                      ...
                      <p className="font-bold text-sm text-[#0D1F17]">
                        ...
                        {s.shape && (
                          <span className="text-[#C9A84C] mr-1">
                            ({s.shape === 'custom' ? s.shapeCustom : s.shape === 'square' ? 'مربع' : s.shape === 'triangle' ? 'مثلث' : s.shape})
                          </span>
                        )}
                      </p>
                      <p className="text-xs text-[#1B5E3B]">استهلاك الثوب: {s.fabricConsumption} سم</p>
                    </div>
                  ))}
                </div>
              )}
                ...
                <div className="mb-3">
                  ...
                  {formajaSeddars.map((s, idx) => (
                    <div key={s.id} className="mb-2 rounded-lg bg-[#FFF8E1] border border-[#C9A84C]/20 p-3">
                      <p className="font-bold text-sm text-[#C9A84C]">🌀 فورمجة {idx + 1}: {s.length} سم</p>
                      <p className="text-xs text-[#1B5E3B]">استهلاك الثوب: {s.fabricConsumption} سم</p>
                    </div>
                  ))}
                </div>
              ...
              <div className="mt-2 flex justify-between items-center rounded-lg bg-[#F5F0E8] px-3 py-2">
                <span className="text-sm font-bold text-[#0D1F17]">مجموع استهلاك الثوب في السدادر</span>
                <span className="font-extrabold text-[#1B5E3B]">{seddarsFabricM} متر</span>
              </div>
              {/* ✅ ثمن ثوب السدادر */}
              <div className="mt-1 flex justify-between items-center rounded-lg bg-white/50 px-3 py-2">
                <span className="text-sm text-gray-600">ثمن الثوب:</span>
                <span className="font-bold text-[#1B5E3B]">{fabricCost.toLocaleString("fr-MA")} DH</span>
              </div>
          ...
          {stitches.length === 0 ? (
            ...
            <>
              {stitches.map((s) => {
                ...
                return (
                  <DetailRow key={s.seddariId} label={`${label}: ${s.styleName}`} value={`${s.finalPrice.toLocaleString("fr-MA")} DH`} highlight={s.finalPrice !== s.basePrice} />
                );
              })}
              <div className="mt-2 flex justify-between items-center rounded-lg bg-[#F5F0E8] px-3 py-2">
              ...
              {draft.stage3TotalOverride !== null && <p className="text-xs text-amber-600 mt-1">⚠️ مُعدّل يدوياً</p>}
            </>
          )}
        </Section>
        ...
        <Section title="🛏️ 4. المخاد" color="#1B5E3B">
          {cushions.length === 0 ? (
            ...
            <>
              {cushions.map((c, idx) => {
                const details = calcCushionDetails(c);
                return (
                  <div key={c.id} className="mb-2 rounded-lg bg-white/50 p-3">
                    <p className="font-bold text-sm text-[#0D1F17]">مخدة {idx + 1}: حجم {c.size}سم × {c.count}</p>
                    <p className="text-xs text-gray-500">
                      سعر المخدة الواحدة: <span className="font-bold text-[#1B5E3B]">{details.pricePerPillow.toFixed(2)} DH</span>
                      <span className="text-gray-400"> (ثوب: {details.fabricCostPerPillow.toFixed(2)} + خياطة: {c.stitchFinalPrice})</span>
                    </p>
                    {c.hasLwata && (
                      <p className="text-xs text-gray-500">لواط: {c.count} × {c.lwataPrice} DH = {details.lwataCost.toFixed(2)} DH</p>
                    )}
                    <p className="text-xs font-bold text-[#1B5E3B]">= {details.total.toLocaleString("fr-MA")} DH</p>
                  </div>
                );
              })}
              <div className="mt-2 flex justify-between items-center rounded-lg bg-[#F5F0E8] px-3 py-2">
                <span className="text-sm font-bold text-[#0D1F17]">مجموع ثمن المخاد</span>
                <span className="font-extrabold text-[#1B5E3B]">{cushionsTotal.toLocaleString("fr-MA")} DH</span>
              </div>
              {draft.stage4TotalOverride !== null && <p className="text-xs text-amber-600 mt-1">⚠️ مُعدّل يدوياً</p>}
            </>
          )}
        </Section>
        ...
        <Section title="🌀 5. مخاد الكيدور" color="#1B5E3B">
          {decor.length === 0 ? (
            ...
            <>
              {decor.map((d, idx) => {
                ...
                return (
                  <div key={d.id} className="mb-2 rounded-lg bg-white/50 p-3">
                    <p className="font-bold text-sm text-[#0D1F17]">كيدور {idx + 1}: {d.shapeName} × {d.count} قطعة</p>
                    <p className="text-xs text-gray-500">خياطة: {d.stitchStyleName} ({d.stitchFinalPrice} DH/قطعة)</p>
                    <p className="text-xs font-bold text-[#1B5E3B]">= {total.toLocaleString("fr-MA")} DH</p>
                  </div>
                );
              })}
              <div className="mt-2 flex justify-between items-center rounded-lg bg-[#F5F0E8] px-3 py-2">
              ...
              {draft.stage5TotalOverride !== null && <p className="text-xs text-amber-600 mt-1">⚠️ مُعدّل يدوياً</p>}
            </>
          )}
        </Section>

        {/* المرحلة 6: الإضافات ✅ FIXED */}
        <Section title="➕ 6. الإضافات" color="#1B5E3B">
          {extrasStage?.lhayef?.enabled && (
            <DetailRow label={`اللحايف (${extrasStage.lhayef.lengthM} متر)`} value={`${(extrasStage.lhayef.totalOverride ?? (extrasStage.lhayef.lengthM * extrasStage.lhayef.pricePerMeter)).toFixed(2)} DH`} />
          )}
          {extrasStage?.tabouria?.enabled && (
            <DetailRow label={`الطابورية (${extrasStage.tabouria.count})`} value={`${(extrasStage.tabouria.totalOverride ?? (extrasStage.tabouria.count * extrasStage.tabouria.unitPrice)).toFixed(2)} DH`} />
          )}
          {/* ✅ عرض العناصر المخصصة */}
          {extrasStage?.customItems && extrasStage.customItems.length > 0 && (
            <>
              {extrasStage.customItems.map((item, idx) => (
                <DetailRow key={item.id} label={`✨ ${item.name}`} value={`${item.price.toFixed(2)} DH`} />
              ))}
            </>
          )}
          {(!extrasStage || (!extrasStage.lhayef?.enabled && !extrasStage.tabouria?.enabled && (!extrasStage.customItems || extrasStage.customItems.length === 0))) && (
        ...
        <div className="rounded-2xl border-2 border-[#C9A84C] bg-white p-6 shadow-xl">
          <div className="flex flex-col gap-3">
            <div className="flex justify-between items-center text-sm text-gray-500">
              <span>ثمن الثوب:</span>
              <span>{fabricCost.toLocaleString("fr-MA")} DH</span>
            </div>
            <div className="flex justify-between items-center text-sm text-gray-500">
      ...
      {pinOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="relative mx-4 w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl">
            <button onClick={() => { setPinOpen(false); setPinOk(false); setPinAction(null); }} className="absolute left-3 top-3 rounded-full bg-gray-100 p-2 text-gray-600 hover:bg-gray-200 transition">
              <RotateCcw className="h-5 w-5" />
            </button>
            <h3 className="mb-4 text-center text-lg font-bold text-[#0D1F17]">
              {pinAction === "editTotal" ? "تعديل المجموع الكلي" : "إضافة إلى السلة"}
            </h3>
            {!pinOk ? (
              <div className="py-4">
                <p className="mb-4 text-center text-gray-600">أدخل كود المدير</p>
                <PinLock role="admin" onSuccess={handlePinSuccess} onCancel={() => { setPinOpen(false); setPinOk(false); setPinAction(null); }} />
              </div>
            ) : pinAction === "editTotal" ? (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input type="number" dir="ltr" value={manualTotalInput} onChange={(e) => setManualTotalInput(e.target.value)} className="w-full rounded-xl border border-gray-300 px-4 py-3 text-lg text-[#0D1F17] focus:border-[#1B5E3B] focus:outline-none focus:ring-2 focus:ring-[#1B5E3B]/20" autoFocus />
                </div>
                <button onClick={handleSaveTotal} disabled={!manualTotalInput || Number(manualTotalInput) < 0} className="w-full rounded-xl bg-[#1B5E3B] py-3 text-base font-bold text-white transition hover:bg-[#144d30] disabled:opacity-40">💾 حفظ</button>
              </div>
            ) : (
              <div className="space-y-4 text-center">
                <p className="text-gray-600">جاري إضافة الطلبية إلى السلة...</p>
              </div>
            )}
          </div>
        </div>
